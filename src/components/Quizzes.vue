...
export default {
    ...
    data() {
      return {
        ...
        items: [],
        timeTest: null,
        alertMessage: 'Calling APIs Sucessful !'
      }
    },
    methods: {
      select() {
    ...
    mounted() {
      axios.get(`https://fb690f6085ad.ngrok.io/getStudentList`)
      .then(response => { this.items = response.data }).then(()=> {window.alert(this.alertMessage)})

      axios.get(`https://fb690f6085ad.ngrok.io/getDeThi`)
      .then(response => { this.options = response.data }).then(()=> {window.alert(this.alertMessage)})
    }      
}
</script>
